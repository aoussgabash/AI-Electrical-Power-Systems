(() => {
  ...
  if (!document.getElementById('course-navigation-runtime-style')) {
    ...
    style.textContent = `
      ...
      .course-action-panel{display:grid!important;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px!important;align-items:stretch!important}
      .course-action-card{min-height:128px;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:13px;padding:16px;border:1px solid rgba(255,255,255,.13);border-radius:15px;background:linear-gradient(145deg,#102d48,#0a1d31);color:#f8fbff!important;text-decoration:none!important;text-align:left;box-shadow:0 10px 24px rgba(0,0,0,.18);transition:transform .2s ease,border-color .2s ease,box-shadow .2s ease;cursor:pointer}
      .course-action-card:hover,.course-action-card:focus-visible{transform:translateY(-3px);border-color:rgba(56,189,248,.65);box-shadow:0 16px 32px rgba(0,0,0,.28);outline:none}
      .course-action-current{cursor:default;border-color:rgba(56,189,248,.38)}.course-action-current:hover{transform:none}
      .course-action-icon{font-size:2rem;line-height:1}.course-action-content{display:flex;flex-direction:column;min-width:0}.course-action-content strong{font-size:1.08rem;color:#fff;line-height:1.35}
      .course-action-subtitle{direction:rtl;text-align:left;color:#dceaf6;font-size:.94rem;line-height:1.5}.course-action-meta{color:#9fb7ca;font-size:.78rem;line-height:1.45;margin-top:5px}.course-action-arrow{font-size:1.3rem;color:#fde047}
      .course-action-related{border-color:rgba(250,204,21,.35)}.course-action-quiz{border-color:rgba(34,197,94,.4)}.course-action-download{border-color:rgba(56,189,248,.5);background:linear-gradient(145deg,#0d4770,#0a2944)}
      @media(max-width:620px){.course-author-signature{border-radius:13px;text-align:center;padding:9px 12px}.course-action-panel{grid-template-columns:1fr!important}.course-action-card{min-height:108px}}
  ...
  let actions = document.querySelector('.course-action-panel');
  if (!actions) {
    ...
    actions.appendChild(card({
      tag: 'div', className: 'course-action-current', icon: type === 'lecture' ? '📖' : '⚡',
      title: `${label.en} ${num}`, subtitle: `${label.ar} ${num}`,
      meta: `⏱ ${duration} min · 📊 ${difficulty[0]} | ${difficulty[1]}`
    }));

    actions.appendChild(card({
      className: 'course-action-related', href: relatedFileName(number), icon: type === 'lecture' ? '⚡' : '📖',
    ...
    const download = card({
      className: 'course-action-download', href: `pdf/${type}${num}.pdf`, icon: '📥',
  ...
  queueMicrotask(() => {
    ...
    quiz.id ||= 'quiz';
    const quizLink = card({
      className: 'course-action-quiz', href: '#quiz', icon: '📝', title: 'Quiz', subtitle: 'ابدأ الاختبار',
  ...
  if (!document.querySelector('.course-page-navigation')) {
    ...
    if (number > 1) {
      nav.appendChild(makeLink('course-nav-link previous', fileName(number - 1), '← Previous | السابق', `${label.en} ${String(number - 1).padStart(2, '0')} | ${label.ar} ${String(number - 1).padStart(2, '0')}`));
    } else {
    ...
    if (number < maxNumber) {
      nav.appendChild(makeLink('course-nav-link next', fileName(number + 1), 'Next | التالي →', `${label.en} ${String(number + 1).padStart(2, '0')} | ${label.ar} ${String(number + 1).padStart(2, '0')}`));
    } else {
      const placeholder = document.createElement('div');
      placeholder.className = 'course-nav-placeholder';
      nav.appendChild(placeholder);
